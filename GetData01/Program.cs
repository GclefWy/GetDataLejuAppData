        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    string listsQueryString = @"?city=sh&appkey=2408231234&page=" + index.ToString() + "&pcount=50";
                    ...
                    if (rtn.Length > 0)
                    {
                        ...
                        for (int i = 0; i < jo["entry"].Count(); i++)
                        {
                            ...
                            string sql = "delete from TB_ESTATE_LISTS_LEJUAPP where lists_hid = '" + d_hid + "'; ";
                            sql += "insert into TB_ESTATE_LISTS_LEJUAPP ([lists_house_id],[lists_hid],[lists_salestate],[lists_price_avg],[lists_name],[lists_address],[lists_main_housetype],[lists_pic_s],[lists_price_display],[lists_district],[lists_price_time],[lists_price_trend],[lists_price_rate]) ";
                            sql += "values ('" + d_house_id + "','" + d_hid + "','" + d_salestate + "','" + d_price_avg + "','" + d_name + "','" + d_address + "','" + d_main_housetype + "','" + d_pic_s + "','" + d_price_display + "','" + d_district + "','" + d_price_time + "','" + d_price_trend + "','" + d_price_rate + "'); ";
